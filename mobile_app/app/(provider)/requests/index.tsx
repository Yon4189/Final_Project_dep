        ...
        renderItem={({ item }) => (
          ...
          >
            <Ionicons
              name={item.icon}
              size={14}
              color={activeFilter === item.value ? colors.surface : colors.text.secondary}
            />
          ...
          <View style={styles.modalBody}>
            ...
            <View style={styles.filterOptions}>
              {FILTERS.map((filter) => (
                <TouchableOpacity
                  key={filter.value}
                  style={[styles.filterOption, activeFilter === filter.value && styles.filterOptionActive]}
                  onPress={() => { setActiveFilter(filter.value); setShowFilterModal(false); }}
                >
                  <Ionicons
                    name={filter.icon}
                    size={18}
                    color={activeFilter === filter.value ? colors.primary : colors.text.secondary}
                  />
                  <Text style={[styles.filterOptionText, activeFilter === filter.value && styles.filterOptionTextActive]}>
                    {filter.label}
                  </Text>
                </TouchableOpacity>
              ))}
...
const getStyles = (colors: ThemeColors, isDark: boolean) => StyleSheet.create({
  ...
  searchContainer: { flexDirection: "row", alignItems: "center", backgroundColor: colors.background, marginHorizontal: 20, marginBottom: 16, paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: colors.border },
  searchInput: { flex: 1, marginLeft: 10, fontSize: 14, color: colors.text.primary, padding: 0 },
  filtersContainer: { paddingHorizontal: 20, gap: 8 },
  filterChip: { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingVertical: 8, backgroundColor: colors.background, borderRadius: 20, marginRight: 8, borderWidth: 1, borderColor: colors.border, gap: 6 },
  filterChipActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  filterChipText: { fontSize: 13, color: colors.text.secondary },
  filterChipTextActive: { color: colors.surface },
  listContainer: { padding: 20, flexGrow: 1 },
  requestCard: { backgroundColor: colors.surface, borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: colors.border },
  ...
  customerName: { fontSize: 15, fontWeight: "600", color: colors.text.primary, marginBottom: 2 },
  requestNumber: { fontSize: 12, color: colors.text.secondary },
  statusBadge: { flexDirection: "row", alignItems: "center", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, gap: 4, marginLeft: 8 },
  statusText: { fontSize: 11, fontWeight: "500", textTransform: "capitalize" },
  serviceInfo: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 },
  serviceName: { fontSize: 14, color: colors.text.primary, flex: 1 },
  priceTag: { backgroundColor: colors.primary + "10", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  priceText: { fontSize: 13, fontWeight: "600", color: colors.primary },
  detailsContainer: { marginBottom: 12 },
  detailRow: { flexDirection: "row", alignItems: "center", marginBottom: 6 },
  detailText: { marginLeft: 8, fontSize: 12, color: colors.text.secondary, flex: 1 },
  cardFooter: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 8, paddingTop: 12, borderTopWidth: 1, borderTopColor: colors.border },
  timeAgo: { fontSize: 11, color: colors.text.secondary },
  actionButtons: { flexDirection: "row", gap: 8 },
  actionButton: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 4 },
  acceptButton: { backgroundColor: colors.success + "10", borderWidth: 1, borderColor: colors.success + "30" },
  acceptButtonText: { color: colors.success, fontSize: 12, fontWeight: "600" },
  rejectButton: { backgroundColor: colors.error + "10", borderWidth: 1, borderColor: colors.error + "30" },
  rejectButtonText: { color: colors.error, fontSize: 12, fontWeight: "600" },
  rescheduleButton: { backgroundColor: colors.warning + "10", borderWidth: 1, borderColor: colors.warning + "30" },
  rescheduleButtonText: { color: colors.warning, fontSize: 12, fontWeight: "600" },
  directionsButton: { backgroundColor: colors.primary },
  directionsButtonText: { color: colors.surface, fontSize: 12, fontWeight: "600" },
  completeButton: { backgroundColor: colors.success },
  completeButtonText: { color: colors.surface, fontSize: 12, fontWeight: "600" },
  ...
  modalBody: { gap: 16 },
  modalLabel: { fontSize: 14, fontWeight: "500", color: colors.text.primary, marginBottom: 4 },
  modalInput: { backgroundColor: colors.background, borderRadius: 12, padding: 12, fontSize: 14, color: colors.text.primary, minHeight: 100, borderWidth: 1, borderColor: colors.border },
  modalPicker: { flexDirection: "row", alignItems: "center", backgroundColor: colors.background, borderRadius: 12, padding: 16, borderWidth: 1, borderColor: colors.border, gap: 12 },
  modalPickerText: { fontSize: 14, color: colors.text.primary },
  modalActions: { flexDirection: "row", justifyContent: "space-between", marginTop: 20, gap: 12 },
  modalCancelButton: { flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1, borderColor: colors.border, alignItems: "center" },
  modalCancelText: { fontSize: 14, color: colors.text.secondary, fontWeight: "500" },
  modalConfirmButton: { flex: 1, backgroundColor: colors.error, paddingVertical: 14, borderRadius: 12, alignItems: "center" },
  modalConfirmText: { fontSize: 14, color: colors.surface, fontWeight: "600" },
  modalButtonDisabled: { opacity: 0.5 },
  filterSectionTitle: { fontSize: 16, fontWeight: "600", color: colors.text.primary, marginBottom: 16 },
  filterOptions: { gap: 12 },
  filterOption: { flexDirection: "row", alignItems: "center", padding: 12, backgroundColor: colors.background, borderRadius: 12, borderWidth: 1, borderColor: colors.border, gap: 12 },
  filterOptionActive: { borderColor: colors.primary, backgroundColor: colors.primary + "10" },
  filterOptionText: { fontSize: 14, color: colors.text.primary },
  filterOptionTextActive: { color: colors.primary, fontWeight: "500" },
